...
func (l *libreswan) createConnections(network *types.Network) error {
	...
	if err := l.createRelayConnections(desiredRelayConns, centralGw); err != nil {
		return err
	}
...
func (l *libreswan) createRelayConnections(desiredRelayConns map[string]*vpndriver.Connection, centralGw *types.Endpoint) error {
	...
	for connName := range l.relayConnections {
		if _, ok := desiredRelayConns[connName]; !ok {
			...
			if centralGw.NodeName == l.nodeName {
				errList = errList.Append(l.deleteRavenSkipNAT(centralGw, l.relayConnections[connName]))
			}
		}
	}
